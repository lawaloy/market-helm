...
describe('AlertsSettings dual-channel Slack format load', () => {
  beforeEach(() => {
  ...
  it('keeps loaded Slack format on an unrelated save without rewriting the webhook URL', async () => {
    await renderWithWatches();

    expect(screen.getByRole('button', { name: 'Slack' }).getAttribute('class')).toContain(
      'bg-[#4A154B]',
    );
    expect(screen.getByRole('button', { name: 'Discord' }).getAttribute('class')).not.toContain(
      'bg-[#5865F2]',
    );

    fireEvent.change(screen.getByPlaceholderText('you@email.com'), {
      target: { value: 'rotated@example.com' },
    });
    ...
    expect(payload.defaults.webhook_format).toBe('slack');
    expect(payload.defaults.notify_email).toBe(true);
    expect(payload.defaults.notify_webhook).toBe(true);
    expect(payload.defaults.email_to).toBe('rotated@example.com');
