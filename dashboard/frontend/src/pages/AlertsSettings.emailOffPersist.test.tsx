...
describe('AlertsSettings Email-off keep-Discord persist', () => {
  beforeEach(() => {
    ...
    apiMocks.testAlert.mockResolvedValue({
      data: { alert_id: dualChannelRule.id, status: 'ok', notifiers: ['webhook'] },
    });
  ...
  it('rewrites dual-channel watches to webhook-only when Email is turned off', async () => {
    ...
    expect(payload.defaults.email_to).toBeUndefined();
