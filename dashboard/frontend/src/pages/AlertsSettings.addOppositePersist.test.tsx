...
describe('AlertsSettings dual-channel composer opposite-operator persist', () => {
  beforeEach(() => {
  ...
  it('auto-saves a complementary rises-above watch without treating it as a duplicate', async () => {
    ...
    expect(payload.alerts.map((alert: { id: string }) => alert.id)).toEqual([
      'aapl_less_than_150',
      'msft_less_than_150',
      'aapl_greater_than_150',
    ]);
    expect(payload.alerts[2]).toEqual(
      expect.objectContaining({
        id: 'aapl_greater_than_150',
        enabled: true,
        cooldown_minutes: 60,
  ...
  it('allows Send test and Check watches now after a complementary composer add', async () => {
    await renderWithWatches();
    ...
    expect(apiMocks.testAlert.mock.calls[0][0]).not.toBe('aapl_greater_than_150');
