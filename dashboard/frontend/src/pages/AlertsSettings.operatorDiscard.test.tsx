...
async function flipOperatorToGreaterThan() {
  ...
  expect(screen.getByText('Rises above')).toBeTruthy();
}

describe('AlertsSettings dual-channel operator discard', () => {
  beforeEach(() => {
  ...
  it('discards an unsaved operator flip without persisting', async () => {
    await renderWithWatches();

    await flipOperatorToGreaterThan();
    fireEvent.click(screen.getByRole('button', { name: 'Discard' }));

    await waitFor(() => {
      expect(screen.queryByRole('button', { name: 'Discard' })).toBeNull();
    });
    expect(screen.getByText('Falls below')).toBeTruthy();
    expect(screen.queryByText('Rises above')).toBeNull();
    expect(apiMocks.saveConfig).not.toHaveBeenCalled();
    expect(apiMocks.testAlert).not.toHaveBeenCalled();
  });

  it('allows Send test and Check watches now after discarding an operator flip', async () => {
    await renderWithWatches();
