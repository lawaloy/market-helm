...
describe('AlertsSettings dual-channel webhook URL persist', () => {
  beforeEach(() => {
    ...
    apiMocks.testAlert.mockResolvedValue({
      data: { alert_id: dualChannelRule.id, status: 'ok', notifiers: ['email', 'webhook'] },
    });
  ...
  it('replaces the stored webhook URL without dropping email on dual-channel watches', async () => {
    await renderWithWatches();

    const rotatedUrl = 'https://hooks.example/rotated-dual';
    fireEvent.change(screen.getByLabelText(/Webhook URL/i), {
      target: { value: rotatedUrl },
    });
    ...
    expect(payload.defaults.webhook_url).toBe(rotatedUrl);
