...
describe('AlertsSettings dual-channel email_to discard', () => {
  beforeEach(() => {
  ...
  it('discards an unsaved email_to draft without persisting', async () => {
    await renderWithWatches();
    const draftAddr = 'unsaved@example.com';
    const emailInput = screen.getByPlaceholderText('you@email.com') as HTMLInputElement;

    fireEvent.change(emailInput, { target: { value: draftAddr } });
    expect(emailInput.value).toBe(draftAddr);
  ...
  it('allows Send test and Check watches now after discarding an email_to draft', async () => {
    await renderWithWatches();
