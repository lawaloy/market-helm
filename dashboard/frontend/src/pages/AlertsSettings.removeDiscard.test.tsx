...
async function removeAaplWatch() {
  const aaplCard = screen.getByRole('switch', { name: 'Enable AAPL price alert' }).closest('li');
  expect(aaplCard).toBeTruthy();
  fireEvent.click(within(aaplCard as HTMLElement).getByTitle('Remove'));
  await waitFor(() => {
    expect(screen.getByRole('button', { name: 'Discard' })).toBeTruthy();
  });
  expect(screen.queryByRole('switch', { name: 'Enable AAPL price alert' })).toBeNull();
  expect(screen.getByRole('switch', { name: 'Enable MSFT price alert' })).toBeTruthy();
}

describe('AlertsSettings dual-channel remove discard', () => {
  beforeEach(() => {
  ...
  it('discards an unsaved remove without persisting', async () => {
    await renderWithWatches();

    await removeAaplWatch();
  ...
  it('allows Send test and Check watches now after discarding a remove', async () => {
    await renderWithWatches();
    ...
    fireEvent.click(within(aaplCard as HTMLElement).getByTitle('Send test'));
    await waitFor(() => {
      expect(apiMocks.testAlert).toHaveBeenCalledTimes(1);
    });
    expect(apiMocks.testAlert.mock.calls[0][0]).toBe(aaplRule.id);
    expect(apiMocks.testAlert.mock.calls[0][0]).not.toBe('msft_less_than_150');
