...
async function deferComposerAdd() {
...
describe('AlertsSettings dual-channel composer add defer', () => {
  beforeEach(() => {
  ...
  it('adds locally without persisting when delivery channels are off', async () => {
    await renderWithWatches();
    await deferComposerAdd();

    expect(apiMocks.saveConfig).not.toHaveBeenCalled();
    expect(screen.getByRole('status').textContent).toContain(
      'AAPL added — finish email or Discord/Slack above, then click Save now at the bottom.',
    );
    expect(screen.getByText('Finish notification setup to save')).toBeTruthy();
    expect((screen.getByRole('button', { name: 'Save now' }) as HTMLButtonElement).disabled).toBe(
      true,
    );
  ...
  it('discards a deferred add without persisting or dropping existing dual-channel watches', async () => {
    await renderWithWatches();
    await deferComposerAdd();
  ...
  it('refuses Send test and Check watches now without auto-saving a deferred add', async () => {
    await renderWithWatches();
  ...
  it('Save now after restoring channels persists the deferred add without replacing siblings', async () => {
    await renderWithWatches();
    await deferComposerAdd();
