...
describe('AlertsSettings auth load errors', () => {
  beforeEach(() => {
  ...
  it('tells hosted users to sign in when Helmtower config returns 401', async () => {
    apiMocks.getConfig.mockRejectedValueOnce(axiosStatus(401));

    render(<AlertsSettings />);

    expect(
      await screen.findByText('Sign in required to access your Helmtower settings.'),
    ).toBeTruthy();
  });

  it('does not treat a 500 config failure as a sign-in requirement', async () => {
    apiMocks.getConfig.mockRejectedValueOnce(axiosStatus(500, 'Network Error'));
