...
describe('AlertsSettings mutation persist', () => {
  ...
  it('persists Slack format when the Slack chip is selected', async () => {
    await renderWithWatches();
    ...
    fireEvent.change(screen.getByLabelText(/Webhook URL/i), {
      target: { value: 'https://hooks.example/slack-rotated' },
    });
    ...
    expect(payload.defaults.webhook_url).toBe('https://hooks.example/slack-rotated');
    expect(payload.alerts).toEqual([
      expect.objectContaining({
        id: 'aapl_less_than_150',
