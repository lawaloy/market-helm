...
describe('AlertsSettings dual-channel Slack format discard', () => {
  beforeEach(() => {
  ...
  it('discards an unsaved Slack chip back to Discord without persisting', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Slack' }).getAttribute('class')).not.toContain(
      'bg-[#4A154B]',
    );
  ...
  it('allows Send test and Check watches now after discarding Slack without saving', async () => {
    await renderWithWatches();
