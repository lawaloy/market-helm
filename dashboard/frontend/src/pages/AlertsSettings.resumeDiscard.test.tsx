...
describe('AlertsSettings dual-channel resume discard', () => {
  beforeEach(() => {
    ...
    apiMocks.testAlert.mockResolvedValue({
      data: {
        alert_id: pausedDualChannelRule.id,
        status: 'sent',
  ...
  it('discards an unsaved resume without persisting', async () => {
    await renderWithWatches();
    const enableSwitch = screen.getByRole('switch', { name: 'Enable AAPL price alert' });
    ...
    expect(
      screen.getByRole('switch', { name: 'Enable AAPL price alert' }).getAttribute('aria-checked'),
    ).toBe('false');
    expect(apiMocks.saveConfig).not.toHaveBeenCalled();
    expect(apiMocks.testAlert).not.toHaveBeenCalled();
  });

  it('allows Send test and Check watches now after discarding a resume', async () => {
    await renderWithWatches();
    ...
    expect(apiMocks.testAlert.mock.calls[0][0]).toBe(pausedDualChannelRule.id);
