...
async function tightenThresholdTo50() {
  ...
  const card = document.querySelector('.alerts-rule-card') as HTMLElement;
  expect(card).toBeTruthy();
  expect(within(card).getByText('$50.00')).toBeTruthy();
}

describe('AlertsSettings dual-channel threshold discard', () => {
  beforeEach(() => {
  ...
  it('discards an unsaved threshold tighten without persisting', async () => {
    await renderWithWatches();

    await tightenThresholdTo50();
    ...
    expect(within(card).queryByText('$50.00')).toBeNull();
    expect(apiMocks.saveConfig).not.toHaveBeenCalled();
    expect(apiMocks.testAlert).not.toHaveBeenCalled();
  });

  it('allows Send test and Check watches now after discarding a threshold tighten', async () => {
    await renderWithWatches();
    ...
    expect(apiMocks.testAlert.mock.calls[0][0]).not.toBe('aapl_less_than_50');
