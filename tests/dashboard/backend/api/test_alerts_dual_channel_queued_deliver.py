"""Hosted dual-channel queued deliver must fire both remaining notifiers.

Remainder-list PUTs (#463 email-drop, #465 webhook-drop) only prove the
channel that is left. An unmodified ``['log','email','webhook']`` watch must
still SendGrid the tenant mailbox AND POST its stored webhook URL. A sibling
must still notify its own URL. Process-wide ``ALERT_EMAIL_TO`` /
...
def multi_user_env(tmp_path, monkeypatch):
    db_path = tmp_path / "dual-channel-queued-deliver.db"
...
def test_queued_deliver_fires_email_and_webhook_without_touching_sibling(
    client,
) -> None:
    """Unmodified dual-channel deliver must SendGrid and POST tenant webhook.

    ``_build_notifiers`` walks the remaining notifications list at send time.
    Keeping both email and webhook must not drop either channel, must not fall
    back to process-wide mailbox/URL env (hosted ``allow_env_*`` is off), and
    must not POST a sibling tenant's destination.
    """
    token_a, user_a = _register(client, "dual-both-a@example.com")
    token_b, user_b = _register(client, "dual-both-b@example.com")
    ...
    with patch("src.alerts.alert_engine.LogNotifier.send", return_value=True) as send_log:
        ...
        with patch(
            ...
            stats = process_job_queue("dual-channel-queued-deliver-worker")
