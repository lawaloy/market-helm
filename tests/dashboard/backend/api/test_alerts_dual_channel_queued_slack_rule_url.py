"""Queued dual-channel deliver must merge Slack format when URL is on the rule.
...
missing-URL branch still POSTs Slack. Queued Discord rule-URL (#482) and
hosted Slack rule-URL send (#479) never walk this split on ``JOB_DELIVER``
together: #482 asserts Discord ``content``, #479 never drains the queue.
Hand-edited / preserved rule-level URLs keep the secret on the alert and
Slack on defaults — a gated merge then falls through to hosted JSON (env
Discord is opted out) or posts Discord ``content`` if env leaked. This locks
that split on queued deliver: POST Slack ``text``/``blocks`` to the rule URL,
still SendGrid the tenant mailbox, sibling unused.
"""
...
def multi_user_env(tmp_path, monkeypatch):
    db_path = tmp_path / "dual-channel-queued-slack-rule-url.db"
...
def test_queued_deliver_merges_slack_format_when_url_is_on_the_rule(client) -> None:
    ...
    if env leaked — or skip the webhook if the rule URL is ignored. Queued
    Discord rule-URL (#482) asserts ``content``, not Slack. Hosted Slack
    rule-URL send (#479) never drains ``JOB_DELIVER``. ``send`` must POST Slack
    ``text``/``blocks`` to the rule URL, SendGrid the tenant mailbox, and leave
    sibling/env destinations unused.
    """
    token_a, user_a = _register(client, "dual-queued-slack-rule-url-a@example.com")
    token_b, user_b = _register(client, "dual-queued-slack-rule-url-b@example.com")
    ...
    with patch("src.alerts.alert_engine.LogNotifier.send", return_value=True) as send_log:
        ...
        with patch(
            ...
            stats = process_job_queue("dual-channel-queued-slack-rule-url-worker")
