"""Hosted dual-channel /api/alerts/test dry-run must preview Slack without sending.

Live dual-channel Slack Send test (#476) walks ``run_alert_test`` with
``dry_run=False`` and POSTs Slack ``text``/``blocks`` plus SendGrid. The
dry-run branch builds previews via ``build_payload`` and must not call
``send`` — otherwise a preview request ships tenant Slack/email traffic.
Discord dry-run (#475) only locks Discord ``content``; deleting the
Slack merge on this path would still pass it (and would still pass #476,
which always ``send``s). This locks the hosted Slack preview: stored
format still merges (not env Discord / hosted JSON fallback), no trigger
claim, no jobs, and sibling/env URLs stay unused.
"""
...
def multi_user_env(tmp_path, monkeypatch):
    db_path = tmp_path / "dual-channel-slack-dry-run.db"
...
def test_hosted_dry_run_previews_slack_without_sending(client) -> None:
    """Dry-run must preview Slack text/blocks and skip HTTP, cooldown, and jobs.

    ``run_alert_test`` merges defaults then ``_build_notifiers``. A missed
    Slack merge would preview JSON ``alert_id`` (hosted fallback) or Discord
    ``content`` if env leaked. ``send`` must not run: SendGrid, the tenant
    webhook, ``DISCORD_WEBHOOK_URL``, and the sibling URL stay unused,
    ``last_triggered`` stays unset, and delivery status stays empty.
    """
    token_a, user_a = _register(client, "dual-slack-dry-run-a@example.com")
    token_b, user_b = _register(client, "dual-slack-dry-run-b@example.com")
