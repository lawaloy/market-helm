"""Hosted dual-channel PUTs must keep email when webhook is dropped.

Dropping webhook from a watch that also has email (#458 is webhook-only→log;
#463 is email-drop keeping webhook) must still SendGrid the tenant mailbox and
must not POST the stored webhook URL. A sibling tenant must still notify its
own URL. Process-wide ``ALERT_EMAIL_TO`` / ``ALERT_WEBHOOK_URL`` must not leak
into hosted sends.
"""
...
def multi_user_env(tmp_path, monkeypatch):
    db_path = tmp_path / "dual-channel-webhook-drop.db"
...
def test_put_drops_webhook_keeps_email_queued_deliver_without_touching_sibling(
    client,
) -> None:
    """email+webhook → email after enqueue must SendGrid, not POST webhook.

    ``_build_notifiers`` re-reads the remaining notifications list at deliver
    time. Dropping only webhook must not drop email, must not POST the still-
    stored tenant webhook URL, and must not fall back to process-wide
    mailbox/URL env (hosted ``allow_env_*`` is off).
    """
    token_a, user_a = _register(client, "dual-wh-drop-a@example.com")
    token_b, user_b = _register(client, "dual-wh-drop-b@example.com")
    ...
    with patch("src.alerts.alert_engine.LogNotifier.send", return_value=True) as send_log:
        ...
        # covers both POSTs. Split by URL so a leftover webhook send cannot hide
        # behind the SendGrid mock.
        with patch(
            "src.alerts.notifiers.webhook_notifier.requests.post",
            side_effect=_post_side_effect,
        ) as mock_post:
            stats = process_job_queue("dual-channel-webhook-drop-worker")
