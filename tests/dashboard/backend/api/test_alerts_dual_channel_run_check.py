...
``ALERT_EMAIL_TO`` / ``ALERT_WEBHOOK_URL``, must not POST a sibling tenant's
URL, must not enqueue worker jobs, and must write a trigger so cooldown
swallows a second click.
"""
...
def test_hosted_run_fires_email_and_webhook_and_claims_cooldown(client) -> None:
    ...
    SendGrid and the stored webhook must fire, the sibling URL must stay
    unused, worker jobs must not be enqueued, and a second click during
    cooldown must not send again.
    """
