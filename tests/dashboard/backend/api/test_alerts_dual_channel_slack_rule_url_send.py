"""Hosted live Slack send must merge format when the webhook URL is on the rule.

Slack dry-run with a rule-level URL (#478) never calls ``send``. Live Slack
(#476) puts URL and format on ``defaults`` together, so nesting the format
copy inside the missing-URL branch still POSTs Slack. Hand-edited / preserved
rule-level URLs keep the secret on the alert and the format on defaults —
a gated merge then falls through to hosted JSON (env Discord is opted out)
or posts to the env Discord URL if isolation leaked. This locks the live
split: rule URL + defaults Slack on both ``/run`` and ``/test``.
"""
...
def multi_user_env(tmp_path, monkeypatch):
    db_path = tmp_path / "dual-channel-slack-rule-url-send.db"
...
def test_hosted_run_posts_slack_when_url_is_on_the_rule(client) -> None:
    """Live Check watches now must POST Slack to the rule URL, not env Discord.

    ``apply_alert_defaults`` copies URL and format independently. Gating format
    on a missing URL would still pass #476 (both live on defaults) but this
    POST would be JSON ``alert_id`` (hosted fallback) or Discord ``content``
    if env leaked. Dry-run (#478) never ``send``s, so it cannot lock the
    destination. Email must still SendGrid the tenant mailbox.
    """
    seeded = _seed_dual_tenants(client, "dual-slack-rule-url-run")
    snapshot = (
        "2026-08-27",
        {"AAPL": 140.0},
...
def test_hosted_test_posts_slack_when_url_is_on_the_rule(client) -> None:
    """Send test must POST Slack ``(test)`` to the rule URL and skip cooldown.

    ``run_alert_test`` merges then ``send``s. A missed format copy would POST
    JSON or env Discord ``content`` while #478's dry-run preview still passed.
    The payload must include ``(test)``, must not claim ``last_triggered``,
    and must still SendGrid the tenant mailbox without touching the sibling.
    """
    seeded = _seed_dual_tenants(client, "dual-slack-rule-url-test")
    with patch("src.alerts.alert_engine.LogNotifier.send", return_value=True) as send_log:
        with patch(
            "src.alerts.notifiers.webhook_notifier.requests.post",
            side_effect=_post_side_effect,
