"""Queued evaluate must retarget a dual-channel watch without HTTP leaks.

Log-only retarget (#455) only observes ``LogNotifier``. Dual-channel delete
evaluate (#487) drops the row entirely. Threshold evaluate (#486) keeps the
same symbol under a tighter bar. A Settings save that keeps the same alert
id but swaps AAPL → GOOG must skip the already-queued AAPL tick — no
SendGrid, no tenant webhook POST, no log for that symbol — then deliver
the GOOG tick to the tenant mailbox and webhook, and still POST the sibling
tenant webhook without env Discord/Slack/mailbox leak. Tenant defaults stay
on the PUT so a leftover AAPL index row would still have credentials to fire.
"""
...
def multi_user_env(tmp_path, monkeypatch):
    db_path = tmp_path / "dual-channel-queued-retarget-eval.db"
...
def test_queued_evaluate_retargets_dual_channel_without_touching_sibling(
    client,
) -> None:
    """In-flight evaluate after a same-id symbol swap must skip the old ticker.

    ``list_watches_for_symbol('AAPL')`` must no longer return the dual-channel
    row; the same ``alert_id`` must index under GOOG so the new tick still
    SendGrid + POSTs. Log-only retarget never observes HTTP, so a merge that
    left AAPL indexed with tenant defaults would still pass those tests.
    Enqueue ticks first, then PUT the swapped symbol so the worker sees the
    post-save index. Sibling webhook-only must still POST its URL.
    """
    token_a, user_a = _register(client, "dual-retarget-eval-a@example.com")
    token_b, user_b = _register(client, "dual-retarget-eval-b@example.com")
    ...
    assert get_watch(user_a, "price_watch")["defaults"]["email_to"] == addr_a
    assert get_watch(user_a, "price_watch")["defaults"]["webhook_url"] == url_a
    assert get_watch(user_a, "price_watch")["alert"]["condition"]["symbol"] == "AAPL"
    ...
    with patch("src.alerts.alert_engine.LogNotifier.send", return_value=True) as send_log:
        ...
        with patch(
            ...
            stats = process_job_queue("dual-channel-queued-retarget-eval-worker")
    ...
    assert log_event["alert_id"] == "price_watch"
